// MOV Type 3: ako Zo == 1111, kopira niz od N elemenata pocevsi od adrese Y do adrese X
// N je u drugoj reci instrukcije
// Format: 0000 XXXX YYYY 1111 | N15..N0
...
module cpu #(
    ...
    // N iz druge reci (broj elemenata za kopiranje)
    wire [DATA_WIDTH-1:0] ir_n = ir_out[31:16];
    ...
    always @(*) begin
        ...
        case (state_q)
            ...
            // MOV Type 3 (Zo==1111) je dvorecan -> ide na S_FETCH_MAR_2
            S_DECODE: begin
                if (opcode == OP_MOV && Zo == 4'b1111)
            ...
            S_EAY_DONE: begin
                ...
                if (opcode==OP_MOV && Zo==4'b0000) begin
                    state_d = S_RDY_MAR;
                end else if (opcode==OP_MOV && Zo==4'b1111) begin
                    // MOV Type 3: kopiraj N elemenata od Y do X
                    state_d = S_MOVN_START;
            ...
            // MOV Type 3: inicijalizacija petlje, N iz druge reci
            S_MOVN_START: begin
                loop_cnt_d   = {DATA_WIDTH{1'b0}};
                loop_total_d = ir_n;
                state_d      = S_MOVN_RD_MAR;
            end
            ...
            S_MOVN_RD_MAR: begin
                ...
                state_d  = S_MOVN_RD_MDR;
            end

            S_MOVN_RD_MDR: begin
                state_d = S_MOVN_RD_MDR_W;
            end

            S_MOVN_RD_MDR_W: begin
                mdr_in_d = mem;
                mdr_ld_d = 1'b1;
                state_d  = S_MOVN_WR;
            end
