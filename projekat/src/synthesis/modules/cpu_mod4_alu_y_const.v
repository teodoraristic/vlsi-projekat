// ADD/SUB/MUL sa konstantom kao drugi operand (Y)
// Ako je Yo == 0000, Y operand je konstanta iz druge reci instrukcije
// Format: 0op XXXX 0000 ZZZZ | C15..C0
// Rezultat: mem[X] = C op mem[Z]
...
module cpu #(
    ...
    localparam [7:0]
        ...
        S_FETCH_IR_2    = 8'd43
        // Nema novih stanja - koristimo postojeci ALU tok
        // Konstanta se smesta u Y_val pre ALU_EXEC
    ;
    ...
    // Da li je trenutna instrukcija ALU sa Y konstantom
    wire is_alu_yconst = (opcode==OP_ADDC || opcode==OP_SUBC ||
    ...
    always @(*) begin
        ...
        case (state_q)
            ...
            // ALU sa Y konstantom su dvoreche instrukcije
            S_DECODE: begin
            ...
            // Za ALU sa Y konstantom, Yo==0000 pa preskacemo citanje Y iz mem
            S_EAY_START: begin
                if (Yo == 4'b0000) begin
            ...
            S_EAY_DONE: begin
                ...
                if (opcode==OP_MOV && Zo==4'b0000) begin
                    ...
                    stop_phase_d = 2'd0;
                    state_d      = S_STOP_NEXT;
                end else if (is_alu_yconst) begin
                    // Y je konstanta iz druge reci, odmah idemo na citanje Z
                    Y_val_d = ir_const;
                    state_d = S_RDZ_MAR;
                end else begin
                    state_d = S_EAZ_START;
            ...
            // Y_val_q vec sadrzi pravu vrednost (iz mem ili iz ir_const)
            S_ALU_EXEC: begin
                case (opcode)
