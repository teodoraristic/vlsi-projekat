// MOV Type 4: ako Zo[3]==0 i Zo[2:0]>0, kopira niz
// N (broj elemenata) se nalazi u memoriji na adresi Zo[2:0]
// Format: 0000 XXXX YYYY 0NNN  (NNN = adresa u FDA gde se cuva duzina niza)
...
module cpu #(
    ...
    localparam [7:0]
        ...
        S_FETCH_IR_2      = 8'd43,
        // MOV Type 4 - citanje N iz memorije
        S_MOVN4_RD_N_MAR  = 8'd44,
        S_MOVN4_RD_N_MDR  = 8'd45,
        S_MOVN4_RD_N_MDR_W= 8'd46,
        S_MOVN4_START     = 8'd47,
        S_MOVN4_RD_MAR    = 8'd48,
        S_MOVN4_RD_MDR    = 8'd49,
        S_MOVN4_RD_MDR_W  = 8'd50,
        S_MOVN4_WR        = 8'd51
    ;
    ...
    // Adresa u FDA gde se cuva N
    wire [ADDR_WIDTH-1:0] N_addr = {{(ADDR_WIDTH-3){1'b0}}, Z_reg};
    ...
    // Brojaci za petlju kopiranja
    reg [DATA_WIDTH-1:0] loop_cnt_d,   loop_cnt_q;
    reg [DATA_WIDTH-1:0] loop_total_d, loop_total_q;
    ...
    always @(*) begin
        ...
        case (state_q)
            ...
            // MOV Type 4 je jednorecan (N je u memoriji, ne u drugoj reci)
            S_DECODE: begin
                if (opcode != OP_IN  && opcode != OP_ADD &&
                    opcode != OP_DIV && opcode != OP_MOV &&
                    opcode != OP_MUL && opcode != OP_OUT &&
            ...
            S_EAY_DONE: begin
                ...
                if (opcode==OP_MOV && Zo==4'b0000) begin
                    // Originalni MOV
                    state_d = S_RDY_MAR;
                end else if (opcode==OP_MOV && Zo[3]==1'b0 && Zo[2:0]>3'b000) begin
                    // MOV Type 4: ucitaj N iz memorije na adresi Zo[2:0]
                    state_d = S_MOVN4_RD_N_MAR;
            ...
            // MOV Type 4: ucitaj N iz memorije na adresi Zo[2:0]
            S_MOVN4_RD_N_MAR: begin
                mar_ld_d = 1'b1;
                mar_in_d = N_addr;
                state_d  = S_MOVN4_RD_N_MDR;
            end

            S_MOVN4_RD_N_MDR: begin
                state_d = S_MOVN4_RD_N_MDR_W;
            end

            S_MOVN4_RD_N_MDR_W: begin
                mdr_in_d = mem;
                mdr_ld_d = 1'b1;
                state_d  = S_MOVN4_START;
            end

            // N je sada u mdr_out, inicijalizuj petlju
            S_MOVN4_START: begin
                loop_cnt_d   = {DATA_WIDTH{1'b0}};
                loop_total_d = mdr_out;
                state_d      = S_MOVN4_RD_MAR;
            end

            // Citaj mem[Y + i]
            S_MOVN4_RD_MAR: begin
                mar_ld_d = 1'b1;
                mar_in_d = EA_y_q + loop_cnt_q[ADDR_WIDTH-1:0];
                state_d  = S_MOVN4_RD_MDR;
            end

            S_MOVN4_RD_MDR: begin
                state_d = S_MOVN4_RD_MDR_W;
            end

            S_MOVN4_RD_MDR_W: begin
                mdr_in_d = mem;
                mdr_ld_d = 1'b1;
                state_d  = S_MOVN4_WR;
            end

            // Upiši mem[X + i] = procitana vrednost
            S_MOVN4_WR: begin
                we_d     = 1'b1;
                data_d   = mdr_out;
                mar_ld_d = 1'b1;
                mar_in_d = EA_x_q + loop_cnt_q[ADDR_WIDTH-1:0];
                if (loop_cnt_q == loop_total_q - 1) begin
                    state_d = S_NEXT_FETCH;
                end else begin
                    loop_cnt_d = loop_cnt_q + 1'b1;
                    state_d    = S_MOVN4_RD_MAR;
                end
            end
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            loop_cnt_q   <= {DATA_WIDTH{1'b0}};
            loop_total_q <= {DATA_WIDTH{1'b0}};
