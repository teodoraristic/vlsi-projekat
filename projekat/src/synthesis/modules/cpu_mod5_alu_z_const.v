// ADD/SUB/MUL sa konstantom kao treci operand (Z)
// Ako je Zo == 0000, Z operand je konstanta iz druge reci instrukcije
// Format: 0op XXXX YYYY 0000 | C15..C0
// Rezultat: mem[X] = mem[Y] op C
// Opcodes sa konstantom: ADD=1001, SUB=1010, MUL=1011, DIV=1100
...
module cpu #(
    ...
    // Opcodes sa konstantom (Z=konst)
    localparam [3:0] OP_ADDC = 4'b1001;
    localparam [3:0] OP_SUBC = 4'b1010;
    localparam [3:0] OP_MULC = 4'b1011;
    localparam [3:0] OP_DIVC = 4'b1100;
    ...
    // Konstanta iz druge reci
    wire [DATA_WIDTH-1:0] ir_const = ir_out[31:16];

    // Da li je trenutna instrukcija ALU sa Z konstantom
    wire is_alu_zconst = (opcode==OP_ADDC || opcode==OP_SUBC ||
                          opcode==OP_MULC || opcode==OP_DIVC);
    ...
    always @(*) begin
        ...
        case (state_q)
            ...
            // ALU sa Z konstantom su dvoreche instrukcije
            S_DECODE: begin
                if (opcode==OP_ADDC || opcode==OP_SUBC ||
                    opcode==OP_MULC || opcode==OP_DIVC)
                    state_d = S_FETCH_MAR_2;
                else if (opcode != OP_IN  && opcode != OP_ADD &&
            ...
            // Za ALU sa Z konstantom, Zo==0000 pa preskacemo citanje Z iz mem
            S_EAZ_START: begin
                if (is_alu_zconst) begin
                    // Z je konstanta iz druge reci, preskoci EA_Z
                    EA_z_d  = {ADDR_WIDTH{1'b0}};
                    z_ptr_d = 1'b0;
                    state_d = S_RDY_MAR;
                end else if (Zo == 4'b0000) begin
                    EA_z_d  = {ADDR_WIDTH{1'b0}};
                    z_ptr_d = 1'b0;
                    state_d = S_RDY_MAR;
            ...
            // Za ALU sa Z konstantom preskacemo citanje Z iz memorije
            S_RDZ_MAR: begin
                if (is_alu_zconst) begin
                    // Z je konstanta, odmah idemo na ALU_EXEC
                    Z_val_d = ir_const;
                    state_d = S_ALU_EXEC;
                end else begin
                    mar_ld_d = 1'b1;
            ...
            // ALU_EXEC radi i za normalne i za konstanta varijante
            // Z_val_q vec sadrzi pravu vrednost (iz mem ili iz ir_const)
            S_ALU_EXEC: begin
                case (opcode)
                    OP_ADD:  alu_oc_d = ALU_ADD;
                    OP_SUB:  alu_oc_d = ALU_SUB;
                    OP_MUL:  alu_oc_d = ALU_MUL;
                    OP_ADDC: alu_oc_d = ALU_ADD;
                    OP_SUBC: alu_oc_d = ALU_SUB;
                    OP_MULC: alu_oc_d = ALU_MUL;
